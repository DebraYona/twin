...
class AddOrEditRecollector extends Component {
    ...
    saveData() {
        ...
        fetch(Constants.APIURLBASE + `recollectors/${this.props.match.params.id}`, {
            method: 'PUT',
            body: formData,
        })
        ...
        .then(data => {
            ...
            this.props.history.push('/magicTowns');
        })
        .catch(error => console.log(error))
    }

    fileChangedHandler = (event) => {
        this.setState({actual_photo: event.target.files[0]});
    }

    componentWillMount() {
        fetch(Constants.APIURLBASE + Constants.STICKERS + this.props.match.params.id)
        .then((response) => response.json())
        .then(data => {
            this.setState({data});
...
export default AddOrEditRecollector;
