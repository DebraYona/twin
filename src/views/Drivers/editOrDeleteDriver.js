...
class Forms extends Component {
  ...
  saveData() {
   ...
   // sending backend
    axios.put(Constants.APIURLBASE + `drivers/${this.state.id}`, formData)
    .then(response => {
            ...
            <Col xs="12" sm="12">
                <Card>
                    ...
                    <FormGroup>
                        ...
                        <Input onChange={(e)=>this.handleText(e)} defaultValue={this.state.licence_number} type="text" id="licence_number" name="licence_number"  />
                        <Label htmlFor="name">Tipo de Licencia</Label>
                        <Input onChange={(e)=>this.handleText(e)} defaultValue={this.state.type_licence} type="select" id="licenceTypeId" name="licenceTypeId" >
                            <option value="">Selecciona una opción</option>
                            {
                                licenceTypes.map(licenceType => (
                                  (this.state.type_licence == licenceType.id) ?
                                    <option key={licenceType.id} value={licenceType.id} selected>{licenceType.type}</option>
                                  :
                        ...
                        <div>
                          <img src={`${Constants.APIURLBASE}${this.state.photo}`} width='50' id="actual_photo" name="actual_photo" />
                        </div>
                        <Label htmlFor="name">Tipo de vehículo asignado</Label>
                        <Input onChange={(e)=>this.handleText(e)} defaultValue={this.state.vahicle_type_id} type="select" id="vehicleTypeId" name="vehicleTypeId" >
                            <option value="">Selecciona una opción</option>
                            {
                                vehicleTypes.map(vehicleType => (
                                    (this.state.vahicle_type_id == vehicleType.id) ?
                                      <option key={vehicleType.id} value={vehicleType.id} selected>{vehicleType.name}</option>
                                    :
                        ...
                        <Input onChange={(e)=>this.handleText(e)} defaultValue={this.state.license_plate} type="text" id="license_plate" name="license_plate"  />
                        <Label htmlFor="status">Estatús</Label>
                        <Input onChange={(e)=>this.handleText(e)} defaultValue={this.state.status} type="text" id="status" name="status" readOnly />
                    </FormGroup>
                    </CardBody>
                    <CardFooter>
